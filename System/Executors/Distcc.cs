...
namespace BuildTool
{
    ...
    internal sealed class Distcc : ActionExecutor
	{
        ...
        public override string OutputName => "Distcc";
        ...
        public Distcc()
		{
			...
			{
				string UserDir                  = Environment.GetEnvironmentVariable(Tag.EnvVar.HOME);
				string MacDistccExecutablesPath = UserDir + Tag.Directory.MacDistcc;
				...
				if (System.IO.File.Exists(UserDir + Tag.Directory.MacDistCodePlist))
				{
		...
		public override bool ExecuteActions(List<Action> Actions, bool bLogDetailedActionStats)
		{
			...
			if (0 < Actions.Count)
			{
				...
				string NumUBTBuildTasks = Environment.GetEnvironmentVariable(Tag.EnvVar.NumBuildToolTasks);

				if (int.TryParse(NumUBTBuildTasks, out int MaxUBTBuildTasks))
				{
				...
				else if (BuildHostPlatform.Current.Platform == BuildTargetPlatform.Mac)
				{
					using (System.Diagnostics.Process DefaultsProcess = new System.Diagnostics.Process())
					{
						try
						{
							...
							string Output = DefaultsProcess.StandardOutput.ReadToEnd();
							DefaultsProcess.WaitForExit();
							if (DefaultsProcess.ExitCode == 0 && int.TryParse(Output, out MaxUBTBuildTasks))
							{
								MaxActionsToExecuteInParallel = MaxUBTBuildTasks;
							}
						}
				...
				else if (System.IO.File.Exists(HostsInfo))
				{
					...
					while ((Line = File.ReadLine()) != null)
					{
						...
						if (HostInfo.Count() == 3)
						{
							if (System.Int32.TryParse(HostInfo[1], out int NumCPUs))
							{
				...
				{
					Environment.SetEnvironmentVariable(Tag.EnvVar.DistccFallback, "0");
				}

				if (bVerboseDistccOutput == true)
				{
					Environment.SetEnvironmentVariable(Tag.EnvVar.DistccVerbose, "1");
				}
						...
						foreach (Action Action in Actions)
						{
							...
							if (bFoundActionProcess == false)
							{
								if (NumExecutingActions < Math.Max(1, MaxActionsToExecuteInParallel))
								{
									...
									{
										bool bFoundPrerequisiteProcess = ActionThreadDictionary.TryGetValue(PrerequisiteAction, out ActionThread PrerequisiteProcess);
