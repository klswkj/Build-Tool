...
namespace BuildTool
{
	...
	internal class HoloLensToolChain : ToolChain
	{
		...
		public HoloLensToolChain(ReadOnlyTargetRules Target)
		{
			...
			// by default tools chains don't parse arguments,
			// but we want to be able to check the -architectures flag defined above.
			// This is only necessary when AndroidToolChain is used during UAT
		...
		private static void AppendCLArguments_Global(CppCompileEnvironment CompileEnvironment, VCEnvironment EnvVars, List<string> Arguments)
		{
			...
			if (CompileEnvironment.Platform != BuildTargetPlatform.HoloLens)
			{
		...
		private static void AppendCLArguments_CPP(CppCompileEnvironment CompileEnvironment, List<string> Arguments)
		{
		...
		private static void AppendCLArguments_C(List<string> Arguments)
		{
			// Explicitly compile the file as C.
			Arguments.Add("/TC");
		...
		static void AppendLibArguments(LinkEnvironment LinkEnvironment, List<string> Arguments)
		{
			...
			// Ignore warning about /ZW in static libraries. It's not relevant since modules
		...
		public override CPPOutput CompileCPPFiles(CppCompileEnvironment CompileEnvironment, List<FileItem> InputFiles, DirectoryReference OutputDir, string ModuleName, IActionGraphBuilder Graph)
		{
			...
			foreach (FileItem SourceFile in InputFiles)
			{
				...
				if (CompileEnvironment.PCHAction == PCHAction.Create)
				{
					...
					string PrecompiledFileExtension = BuildPlatform.GetBuildPlatform(BuildTargetPlatform.HoloLens).GetBinaryExtension(BuildBinaryType.PrecompiledHeader);
					// Add the precompiled header file to the produced items list.
					FileItem PrecompiledHeaderFile = FileItem.GetItemByFileReference(
						FileReference.Combine(OutputDir, Path.GetFileName(SourceFile.AbsolutePath) + PrecompiledFileExtension));
					CompileAction.ProducedItems.Add(PrecompiledHeaderFile);
					Result.PCHFile = PrecompiledHeaderFile;

					// Add the parameters needed to compile the precompiled header file to the command-line.
					FileArguments.AddFormat(" /Yc\"{0}\"", CompileEnvironment.PCHIncludeFilename);
				...
				if (bEmitsObjectFile)
				{
					string ObjectFileExtension = BuildPlatform.GetBuildPlatform(BuildTargetPlatform.HoloLens).GetBinaryExtension(BuildBinaryType.Object);
					// Add the object file to the produced item list.
					FileItem ObjectFile = FileItem.GetItemByFileReference(
						FileReference.Combine(OutputDir, Path.GetFileName(SourceFile.AbsolutePath) + ObjectFileExtension));
				...
				if (CompileEnvironment.bUsePDBFiles)
				{
					...
					if (CompileEnvironment.PCHAction == PCHAction.Include)
					{
						PDBFileName = CompileEnvironment.PCHIncludeFilename.GetFileName();
					}
					...
					FileItem PDBFile = FileItem.GetItemByFileReference(
							FileReference.Combine(OutputDir, PDBFileName + ".pdb"));
					...
					&& CompileAction.ProducedItems.Count > 0)
				{
					...
					FileItem ResponseFileItem = Graph.CreateIntermediateTextFile(ResponseFileName, SharedArguments.Concat(FileArguments).Concat(AdditionalArguments).Select(x => StringUtils.ExpandVariables(x)));
				...
				if (CompileEnvironment.bGenerateDependenciesFile)
				{
					...
					CommandArguments.Add(StringUtils.MakePathSafeToUseWithCommandLine(CompileAction.CommandPath));
					...
					CompileAction.CommandPath = FileReference.Combine(BuildTool.EngineDirectory, "Build", "Windows", "cl-filter", "cl-filter.exe");
				}

				if (CompileEnvironment.PCHAction == PCHAction.Create)
				{
					Log.TraceVerbose("Creating PCH: " + CompileEnvironment.PCHIncludeFilename);
		...
		public override CPPOutput CompileRCFiles(CppCompileEnvironment CompileEnvironment, List<FileItem> InputFiles, DirectoryReference OutputDir, IActionGraphBuilder Graph)
		{
			...
			foreach (FileItem RCFile in InputFiles)
			{
				...
				// Suppress startup header
				List<string> Arguments = new List<string> { "/nologo" };
				...
				if (CompileEnvironment.Architecture == "x64" || 
					CompileEnvironment.Architecture == "ARM64")
				{
		...
		public override FileItem LinkFiles(LinkEnvironment LinkEnvironment, bool bBuildImportLibraryOnly, IActionGraphBuilder Graph) //(LinkEnvironment LinkEnvironment, bool bBuildImportLibraryOnly)
		{
			...
			if (!bIsBuildingLibrary)
			{
				...
				if (LinkEnvironment.ModuleDefinitionFile != null && 0 < LinkEnvironment.ModuleDefinitionFile.Length)
				{
			...
			// file will clobber the import library we went out of our way to generate during an earlier step.
			// This file is not needed for our builds, but there is no way to prevent MSVC from generating it when linking targets that have exports.
			// We don't want this to clobber our LIB file and invalidate the existing timstamp, so instead we simply emit it with a different name
			FileReference ImportLibraryFilePath = FileReference.Combine(LinkEnvironment.IntermediateDirectory, LinkEnvironment.OutputFilePath.GetFileNameWithoutExtension() + ".lib");
			...
			List<string>   InputFileNames    = new List<string>(); // Add the input files to a response file, and pass the response file on the command-line.
            ...
            List<FileItem> ProducedItems = new List<FileItem> { OutputFile };
		...
		private void ExportObjectFilePaths(LinkEnvironment LinkEnvironment, string FileName)
		{
			...
			foreach (string AdditionalLibrary in LinkEnvironment.AdditionalLibraries)
			{
				// Need to handle import libraries that are about to be built (but may not exist yet), third party libraries with relative paths in the tree, and system libraries in the system path
		...
		public static string GetVCIncludePaths(BuildTargetPlatform Platform)
		{
			Debug.Assert(Platform == BuildTargetPlatform.HoloLens);

			// Also add any include paths from the INCLUDE environment variable.
			// MSVC is not necessarily running with an environment that matches what UBT extracted from the vcvars*.bat using SetEnvironmentVariablesFromBatchFile().
			...
			if (IncludePaths.HasValue() && !IncludePaths.EndsWith(";"))
			{
				IncludePaths += ";";
			}
		...
		public override void ModifyBuildProducts(ReadOnlyTargetRules Target, BuildBinary Binary, List<string> Libraries, List<BuildBundleResource> BundleResources, Dictionary<FileReference, BuildProductType> BuildProducts)
		{
			...
			if (Target.Configuration == TargetConfiguration.Development)
			{
