...
namespace UnrealBuildTool
{
	...
	class XcodeProjectFile : ProjectFile
	{
				...
				if (Directory.Exists(ProjectExtensionsDir))
				{
					foreach (DirectoryInfo DI in new System.IO.DirectoryInfo(ProjectExtensionsDir).EnumerateDirectories())
					{
						...
						foreach (XcodeBuildConfig Configuration in BuildConfigs)
						{
							...
							{
								IOSPlatform IOSPlatform = ((IOSPlatform)BuildPlatform.GetBuildPlatform(UnrealTargetPlatform.IOS));
							...
							{
								TVOSPlatform TVOSPlatform = ((TVOSPlatform)BuildPlatform.GetBuildPlatform(UnrealTargetPlatform.TVOS));
		...
		private void AppendShellScriptSection(StringBuilder Content, string ShellScriptGuid, FileReference UProjectPath)
		{
			...
			UProjectDescriptor Project = UProjectDescriptor.FromFile(UProjectPath);
			...
			foreach (PluginInfo PI in AvailablePlugins)
			{
				...
				DirectoryReference FrameworkDir = DirectoryReference.Combine(PI.RootDirectory, "Source/Frameworks");
				if (!DirectoryReference.Exists(FrameworkDir))
				{
					FrameworkDir = DirectoryReference.Combine(PI.RootDirectory, "Frameworks");
				}
				if (DirectoryReference.Exists(FrameworkDir))
				{
					// look at each zip
