...
namespace UnrealBuildTool
{
	...
	class KDevelopGenerator : ProjectFileGenerator
	{
		...
		// Write the primare $ProjectName.kdev4 project file, the one that should be opened when loading the project into KDevelop
...
#pragma warning disable IDE0059 // Unnecessary assignment of a value
		...
		private void WriteIncludeSection(ref StringBuilder FileContent)
		{
			...
			{
				if (!(CurProject is KDevelopProjectFile KDevelopProject))
				{
		...
		private void SplitDefinitionAndValue(string DefinitionText, out String Key, out String Value)
		{
			int EqualsIndex = DefinitionText.IndexOf('=');
			if (0 <= EqualsIndex)
			{
				Key = DefinitionText.Substring(0, EqualsIndex);
				Value = DefinitionText.Substring(EqualsIndex + 1);
			}
			else
			{
				Key = DefinitionText;
