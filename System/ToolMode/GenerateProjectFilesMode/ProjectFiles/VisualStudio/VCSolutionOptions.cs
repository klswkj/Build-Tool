...
namespace BuildTool
{
    class VCBinarySetting
	{
		...
		{
			Name        = InName; // ActiveCfg, StartupProject
		...
		public static VCBinarySetting Read(BinaryReader Reader)
		{
            ...
            {
                // Read the key
                Name = new string(Reader.ReadChars(Reader.ReadInt32())).TrimEnd('\0')
            };
    ...
    internal class VCOleContainer
	{
        ...
        private enum STGC : int
		{
			...
			Overwrite                          = 0x01,
			OnlyIfCurrent                      = 0x02,
			DangerouslyCommitMerelyToDiskCache = 0x04,
			Consolidate                        = 0x08,
		}
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
            try
			{
                ((IOleStorage)null).EnumElements(0, IntPtr.Zero, 0, out IOleEnumSTATSTG Enumerator);
                try
				{
					...
					{
                        ((IOleStorage)null).OpenStream(Stats[Idx].pwcsName, IntPtr.Zero, STGM.Read | STGM.ShareExclusive, 0, out IOleStream OleStream);
                        try
						{
		...
		{
            if (!TryGetSection(Name, out byte[] OutData))
            {
    ...
    internal class VCSolutionExplorerState
	{
		...
		void ReadOpenProjects(BinaryReader Reader, VCProjectFileFormat VSVerison)
		{
			long OpenFoldersEnd = Reader.BaseStream.Position + Reader.ReadInt32();

			if(VCProjectFileFormat.VisualStudio2017 <= VSVerison)
			{
		...
		void WriteOpenProjects(BinaryWriter Writer, VCProjectFileFormat VCVersion)
		{
			if(VCProjectFileFormat.VisualStudio2017 <= VCVersion)
			{
		...
		void ReadEpilogue(BinaryReader Reader)
		{
			...
			if (Reader.ReadInt32() != 1 || 
				ReadString(Reader) != EpilogueGuid || 
				Reader.ReadInt32() != 0)
			{
		...
		int GetStringSize(string Text)
		{
			...
			for (int Length = Text.Length; 127 < Length; Length >>= 7)
			{
				++Size;
			}
			return Size;
	...
	{
        protected readonly VCProjectFileFormat Format;
