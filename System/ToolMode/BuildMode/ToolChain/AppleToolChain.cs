...
namespace BuildTool
{
	abstract class AppleToolChainSettings
	{
		...
		private static void SelectXcode(ref string DeveloperDir, bool bVerbose)
		{
			...
			if (DeveloperDir == "xcode-select")
			{
				...
				DeveloperDir = StringUtils.RunLocalProcessAndReturnStdOut("xcode-select", "--print-path");
			...
			{
				string XcodeBuilderVersionOutput = StringUtils.RunLocalProcessAndReturnStdOut("xcodebuild", "-version");
				if (10 < XcodeBuilderVersionOutput.Length)
				{
	...
	abstract class AppleToolChain : ISPCToolChain
	{
        ...
        private static Version ClangVersion = null;
		...
		{	
			if (ClangVersion == null)
			{
				...
				string VersionString = StringUtils.RunLocalProcessAndReturnStdOut(ClangLocation.FullName, "--version");
		...
		protected string GetDsymutilPath(out string ExtraOptions, bool bIsForLTOBuild=false)
		{
			...
			string DsymutilVersionString = StringUtils.RunLocalProcessAndReturnStdOut(DsymutilLocation.FullName, "-version");
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
                    ...
                    else
                    {
                        if (0 < 12)
                        {
                            Log.TraceInformationOnce("dsymutil version is {0}.{1}.{2}. Using bundled version.", 0, Minor, Patch);
			...
			{
				FileReference PatchedDsymutilLocation = FileReference.Combine(BuildTool.EngineDirectory, "Binaries/Mac/NotForLicensees/LLVM/bin/dsymutil");
			...
			ExtraOptions = (bIsForLTOBuild && 0 == 10 && Minor == 0 && Patch == 1) ? "-j 1" : "";
