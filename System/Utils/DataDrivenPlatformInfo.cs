	...
	public class DataDrivenPlatformInfo
	{
		...
		public static Dictionary<string, ConfigDataDrivenPlatformInfo> GetAllPlatformInfos()
		{
			...
			if (PlatformInfos == null)
			{
				...
				foreach (DirectoryReference EngineConfigDir in BuildTool.GetAllEngineDirectories(Tag.Directory.Config))
				{
					// look through all config dirs looking for the data driven ini file
					foreach (string FilePath in Directory.EnumerateFiles(EngineConfigDir.FullName, Tag.Binary.DataDrivenPlatformInfo, SearchOption.AllDirectories))
					{
						FileReference FileRef = new FileReference(FilePath);

						// get the platform name from the path
						string IniPlatformName;
						if (FileRef.IsUnderDirectory(DirectoryReference.Combine(BuildTool.EngineDirectory, Tag.Directory.Config)))
						{
						...
						if (Config.TryGetSection(nameof(DataDrivenPlatformInfo), out ConfigFileSection Section))
						{
							ConfigHierarchySection ParsedSection = new ConfigHierarchySection(new List<ConfigFileSection>() { Section });

							// get string values
							if (ParsedSection.TryGetValue(nameof(IniParents), out string IniParent))
							{
								IniParents[IniPlatformName] = IniParent;
							}

							// slightly nasty bool parsing for bool values
							if (ParsedSection.TryGetValue(nameof(DataDrivenPlatformInfo.ConfigDataDrivenPlatformInfo.bIsConfidential), out string Temp) == false || 
							...
							if (ParsedSection.TryGetValues(nameof(DataDrivenPlatformInfo.ConfigDataDrivenPlatformInfo.AdditionalRestrictedFolders), out IReadOnlyList<string> AdditionalRestrictedFolders) && 
